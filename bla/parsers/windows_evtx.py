...
# 真实的 Windows EVTX / wevtutil RenderedXml 不会带 DTD 或自定义实体。攻击者却可以
# 伪造一段含 <!DOCTYPE ... [ <!ENTITY ...> ]> 的 XML 来触发实体膨胀（billion laughs）
# 拖垮内存。这里在交给 ElementTree 之前直接拒绝任何 DTD/实体声明，零依赖挡掉该 DoS。
_XML_DOCTYPE_ENTITY_RE = re.compile(r"<!(?:DOCTYPE|ENTITY)\b", re.IGNORECASE)
...
def _parse_xml_event(xml_text: str, source_file: str) -> Optional[LogEvent]:
    ...
    if _XML_DOCTYPE_ENTITY_RE.search(xml_text):
        raise ValueError("拒绝解析含 DTD/实体声明的 XML 事件块（防实体膨胀 DoS）")
