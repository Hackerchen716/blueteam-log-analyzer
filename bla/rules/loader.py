...
def validate_web_attack_rules(rule_dirs: Iterable[str] = ()) -> Dict[str, Any]:
    ...
    for source, raw in raw_rules:
        ...
        mitre_value = str(raw.get("mitre") or raw.get("mitre_attack") or "").strip()
        if mitre_value and not _MITRE_ID_RE.fullmatch(mitre_value):
            issues.append({"severity": "error", "source": source, "rule": rule_name,
                           "message": f"MITRE 技术编号格式非法（应为 T#### 或 T####.###）: {mitre_value}"})
