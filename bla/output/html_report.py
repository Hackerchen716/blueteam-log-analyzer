...
def _incident_brief_html(brief: Dict[str, Any]) -> str:
    if not brief:
        return ""
    headline = brief.get("headline", {})
    facts = brief.get("confirmed_facts", [])[:6]
    findings = brief.get("findings", [])[:4]
    hypotheses = brief.get("hypotheses", [])[:4]
    uncertainties = brief.get("uncertainties", [])[:5]
    boundary = brief.get("evidence_boundary", {})
    timeline = brief.get("key_timeline", [])[:6]
    all_attack_paths = brief.get("attack_paths", [])
    attack_paths = all_attack_paths[:6]
    artifacts = brief.get("suspected_artifacts", [])[:8]
    actors = brief.get("actor_profiles", [])[:8]
    ...
    def evidence_html(items: List[Dict[str, Any]]) -> str:
        ...
        for ev in items[:4]:
            ...
            rows.append(f"<li>{_h(' | '.join(bit for bit in bits if bit))}</li>")
        return "".join(rows) or "<li>暂无证据引用</li>"

    def statement_cards(items: List[Dict[str, Any]]) -> str:
        if not items:
            return '<div class="empty-state">暂无</div>'
        cards = []
        for item in items:
            cards.append(f"""
            <div class="alert-card" style="background:#fff;">
              <div class="alert-header">
                <span class="phase-tag">置信度 {_h(item.get('confidence', 'low'))}</span>
                <strong>{_h(item.get('title', ''))}</strong>
              </div>
              <p class="alert-desc">{_h(item.get('summary', ''))}</p>
              <details><summary>证据引用</summary><ul class="evidence-list">{evidence_html(item.get('evidence', []))}</ul></details>
            </div>""")
        return "".join(cards)
    ...
    for item in timeline:
        ...
        <div class="tl-entry">
          ...
          <div class="tl-content">
            <span class="tl-time">{_h(item.get('time_range_text') or item.get('time_range', ''))}</span>
            <span class="phase-tag">{_h(item.get('phase', ''))}</span>
            <span class="tl-msg">{_h(item.get('summary', ''))}</span>
          </div>
        </div>"""
    if not timeline_html:
        timeline_html = '<div class="empty-state">暂无可归纳案情演化</div>'

    path_cards = ""
    for item in attack_paths:
        ips = ", ".join(item.get("source_ips", [])[:5]) or "?"
        entries = ", ".join(item.get("entry_candidates", [])[:5]) or "?"
        artifacts_text = ", ".join(item.get("artifact_candidates", [])[:5]) or "?"
        path_cards += f"""
        <div class="alert-card" style="background:#fff;">
          <div class="alert-header">
            <span class="phase-tag">{_h(item.get('id', 'path'))}</span>
            <span class="phase-tag">置信度 {_h(item.get('confidence', 'low'))}</span>
            <strong>{_h(item.get('title', '攻击路径'))}</strong>
          </div>
          <p class="alert-desc">{_h(item.get('summary', ''))}</p>
          <p class="muted">时间：{_h(item.get('time_range_text') or item.get('time_range', ''))}；事件：{item.get('event_count', 0)} 条；来源 IP：{_h(ips)}</p>
          <p class="muted">入口候选：{_h(entries)}；文件候选：{_h(artifacts_text)}</p>
          <details><summary>证据引用</summary><ul class="evidence-list">{evidence_html(item.get('evidence', []))}</ul></details>
        </div>"""
    if not path_cards:
        path_cards = '<div class="empty-state">当前证据不足以拆分攻击路径</div>'

    artifact_rows = ""
    for item in artifacts:
        method_status = ", ".join(f"{k}:{v}" for k, v in item.get("method_status", {}).items())
        artifact_rows += f"""
        <tr>
          <td><code>{_h(item.get('path', ''))}</code></td>
          <td>{_h(item.get('confidence', ''))}</td>
          <td>{_h(item.get('first_seen_text') or item.get('first_seen', ''))}</td>
          <td>{_h(item.get('last_seen_text') or item.get('last_seen', ''))}</td>
          <td>{_h(', '.join(item.get('ips', [])[:5]))}</td>
          <td>{_h(method_status)}</td>
        </tr>"""
    if not artifact_rows:
        artifact_rows = '<tr><td colspan="6" class="empty-state">暂无疑似落地文件</td></tr>'

    actor_rows = ""
    for item in actors:
        actor_rows += f"""
        <tr>
          <td><code>{_h(item.get('ip', ''))}</code></td>
          <td>{_h(', '.join(item.get('role_candidates', [])) or '未分类')}</td>
          <td>{_h(item.get('confidence', ''))}</td>
          <td>{item.get('event_count', 0)}</td>
          <td>{_h(item.get('first_seen_text') or item.get('first_seen', ''))}</td>
          <td>{_h(item.get('last_seen_text') or item.get('last_seen', ''))}</td>
        </tr>"""
    if not actor_rows:
        actor_rows = '<tr><td colspan="6" class="empty-state">暂无 IP 行为画像</td></tr>'

    uncertainty_items = "".join(f"<li>{_h(item)}</li>" for item in uncertainties) or "<li>暂无</li>"
    boundary_html = f"""
    <div class="recommendation">
      <strong>证据边界</strong>
      <span>依据：{_h(boundary.get('basis', '当前输入日志'))}。{_h(boundary.get('note', ''))}</span>
    </div>"""
    return f"""
  <h2>应急研判摘要</h2>
  <div class="card">
    <div class="incident-head">
      <span class="badge" style="background:#0e7490;">{_h(headline.get('confidence', 'low'))}</span>
      <strong>{_h(headline.get('title', '未形成明确攻击研判'))}</strong>
    </div>
    <p>{_h(headline.get('summary', ''))}</p>
    <div class="incident-grid">
      <div><h3>已确认事实</h3>{statement_cards(facts)}</div>
      <div><h3>主要发现</h3>{statement_cards(findings)}</div>
    </div>
    <h3 style="margin-top:16px;">案情演化</h3>
    <div class="timeline-card">{timeline_html}</div>
    <h3 style="margin-top:16px;">攻击路径研判（{len(all_attack_paths)} 条）</h3>
    {path_cards}
    <h3 style="margin-top:16px;">研判假设</h3>
    {statement_cards(hypotheses)}
    <h3 style="margin-top:16px;">疑似落地/上传文件</h3>
    <table><thead><tr><th>路径</th><th>置信度</th><th>首次出现</th><th>最近出现</th><th>IP</th><th>方法/状态</th></tr></thead><tbody>{artifact_rows}</tbody></table>
    <h3 style="margin-top:16px;">参与 IP 行为画像</h3>
    <table><thead><tr><th>IP</th><th>角色候选</th><th>置信度</th><th>事件数</th><th>首次</th><th>最近</th></tr></thead><tbody>{actor_rows}</tbody></table>
    <h3 style="margin-top:16px;">不能确认 / 风险边界</h3>
    <ul class="rec-list">{uncertainty_items}</ul>
    {boundary_html}
  </div>"""
...
def generate_html_report(
    ...
    incident_brief = ensure_incident_brief(parse_results, summary)
    ...
    incident_brief_html = _incident_brief_html(incident_brief)
