...
from bla.incident_brief import render_incident_brief_markdown
...
from bla.utils.helpers import (
    SafeArgumentParser,
    escape_markdown_text,
    reset_counter,
    safe_print as print,
    print_sanitized_traceback,
...
def _add_json_output_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--no-json-events",
        action="store_true",
        help="report.json 不写入全量 events，仅保留摘要、告警、案件、时间线和 IOC",
    )
    parser.add_argument(
        "--raw-line-limit",
        type=int,
...
def _validate_json_output_arguments(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.json_events_limit is not None and args.json_events_limit < 0:
        parser.error("--json-events-limit 必须大于等于 0")
    if args.raw_line_limit is not None and args.raw_line_limit < 0:
        parser.error("--raw-line-limit 必须大于等于 0")
    if args.no_json_events and args.json_events_limit is not None:
        parser.error("--no-json-events 不能和 --json-events-limit 同时使用")
...
def _build_manifest_context(args: argparse.Namespace, run_result) -> dict:
    ...
    context["options"] = {
        ...
        "evidence_csv": _manifest_path_label(getattr(args, "evidence_csv", None)),
    }
    return context
...
def _cmd_ssh(argv: List[str]) -> None:
    ...
    try:
        ...
        print(f"❌ 远程工作台启动失败: {sanitize_report_text(e)}", file=sys.stderr)
        sys.exit(2)
    sys.exit(0)
...
def _cmd_remote_log(argv: List[str]) -> None:
    ...
    parser.add_argument("--out", metavar="DIR", help="本地标准报告目录（含 incident_brief.md / incident_evidence.csv / manifest.json）")
    ...
    parser.add_argument("--brief", metavar="FILE", help="本地应急研判摘要 Markdown")
    parser.add_argument("--evidence-csv", metavar="FILE", help="本地应急研判证据包 CSV（仅包含研判摘要引用证据）")
    ...
    _validate_json_output_arguments(args, parser)
    ...
    try:
        ...
        print(f"❌ 远程采集启动失败: {sanitize_report_text(e)}", file=sys.stderr)
        sys.exit(2)
    ...
    _append_optional_arg(remote_args, "--brief", args.brief)
    _append_optional_arg(remote_args, "--evidence-csv", args.evidence_csv)
    _append_optional_arg(remote_args, "--json-events-limit", args.json_events_limit)
    _append_optional_arg(remote_args, "--raw-line-limit", args.raw_line_limit)
    if args.no_json_events:
        remote_args.append("--no-json-events")
...
def _cmd_validate_rules(argv: List[str]) -> None:
    ...
    try:
        ...
        print(f"❌ 规则校验失败: {sanitize_report_text(e)}", file=sys.stderr)
        sys.exit(2)
    ...
    for item in result["issues"][:50]:
        ...
        source = sanitize_report_text(item.get("source", ""))
        rule = sanitize_report_text(item.get("rule", ""))
        message = sanitize_report_text(item.get("message", ""))
        print(f"  [{marker}] {source} :: {rule} :: {message}")
...
def _cmd_explain(argv: List[str]) -> None:
    parser = SafeArgumentParser(
        prog="bla explain",
        description="从 JSON 报告解释研判摘要、alert_id 或 incident_id 的证据与处置建议",
    )
    parser.add_argument("id", help="brief / incident-brief / 告警 ID（如 awa1）或案件 ID（如 inc-001）")
    ...
    try:
        ...
        print(
            f"❌ 无法读取报告 {sanitize_report_text(args.report)}: {sanitize_report_text(e)}",
            file=sys.stderr,
        )
        sys.exit(2)

    target_id = args.id
    if target_id in {"brief", "incident-brief", "incident_brief", "summary", "研判摘要"}:
        brief = report.get("incident_brief") or {}
        if not brief:
            print("❌ 报告中没有 incident_brief；请使用新版 BLA 重新生成 report.json", file=sys.stderr)
            sys.exit(1)
        if args.format == "markdown":
            print(render_incident_brief_markdown(brief))
        else:
            _print_brief_text(brief)
        sys.exit(0)
    ...
    print(f"❌ 报告中未找到 ID: {sanitize_report_text(target_id)}", file=sys.stderr)
    sys.exit(1)


def _print_brief_text(brief: dict) -> None:
    headline = brief.get("headline", {})
    print(f"[应急研判] {_plain(headline.get('title', '未形成明确攻击研判'))}")
    print(f"置信度: {_plain(headline.get('confidence', 'low'))}")
    print(_plain(headline.get("summary", "")))

    print("\n已确认事实:")
    for item in brief.get("confirmed_facts", []):
        print(f"  - {_plain(item.get('title'))}: {_plain(item.get('summary'))}")
    if not brief.get("confirmed_facts"):
        print("  - （无）")

    print("\n主要发现:")
    for item in brief.get("findings", []):
        print(f"  - [{_plain(item.get('confidence', ''))}] {_plain(item.get('title'))}: {_plain(item.get('summary'))}")
    if not brief.get("findings"):
        print("  - （无）")

    print("\n研判假设:")
    for item in brief.get("hypotheses", []):
        print(f"  - [{_plain(item.get('confidence', ''))}] {_plain(item.get('title'))}: {_plain(item.get('summary'))}")
    if not brief.get("hypotheses"):
        print("  - （无）")

    print("\n关键案情演化:")
    for item in brief.get("key_timeline", []):
        print(f"  - {_plain(item.get('phase'))}: {_plain(item.get('time_range_text') or item.get('time_range'))}  {_plain(item.get('summary'))}")
    if not brief.get("key_timeline"):
        print("  - （无）")

    print(f"\n攻击路径研判（{len(brief.get('attack_paths', []))} 条）:")
    for item in brief.get("attack_paths", []):
        ...
        print(
            f"  - [{_plain(item.get('confidence', ''))}] {_plain(item.get('id'))} {_plain(item.get('title'))}: "
            f"{_plain(item.get('time_range_text') or item.get('time_range'))}  "
            f"事件={item.get('event_count', 0)}  IP={_plain(ips)}"
        )
        print(f"    入口候选={_plain(entries)}  文件候选={_plain(artifacts)}")
        if item.get("summary"):
            print(f"    研判={_plain(item.get('summary'))}")
    if not brief.get("attack_paths"):
        print("  - （当前证据不足以拆分攻击路径）")

    print("\n疑似落地/上传文件:")
    for item in brief.get("suspected_artifacts", []):
        method_status = ", ".join(f"{key}:{value}" for key, value in item.get("method_status", {}).items())
        ips = ", ".join(item.get("ips", [])[:5])
        print(
            f"  - [{_plain(item.get('confidence', ''))}] {_plain(item.get('path'))}  "
            f"{_plain(item.get('first_seen_text') or item.get('first_seen'))} ~ {_plain(item.get('last_seen_text') or item.get('last_seen'))}  "
            f"IP={_plain(ips)}  {_plain(method_status)}"
        )
    if not brief.get("suspected_artifacts"):
        print("  - （无）")

    print("\n参与 IP 行为画像:")
    for item in brief.get("actor_profiles", []):
        roles = ", ".join(item.get("role_candidates", [])) or "未分类"
        print(
            f"  - {_plain(item.get('ip'))}: {_plain(roles)}  "
            f"事件={item.get('event_count', 0)}  置信度={_plain(item.get('confidence', 'low'))}"
        )
    ...
    print("\n证据边界:")
    print(f"  - 依据: {_plain(boundary.get('basis', '当前输入日志'))}")
    if boundary.get("note"):
        print(f"  - 说明: {_plain(boundary.get('note'))}")
...
def main():
    ...
    parser.add_argument(
        "--brief",
        metavar="FILE",
        help="生成应急研判摘要 Markdown（适合复盘、工单和客户交付）",
    )
    parser.add_argument(
        "--evidence-csv",
        metavar="FILE",
        help="生成应急研判证据包 CSV（仅包含研判摘要引用证据，不替代 events.csv）",
    )
    _add_json_output_arguments(parser)
    parser.add_argument(
        "--out",
        metavar="DIR",
        help="生成标准报告目录（index.html/report.json/incident_brief.md/incident_evidence.csv/events.csv/iocs.txt/report.sarif/manifest.json）",
    )
    parser.add_argument(
        "--geoip-cache",
        metavar="FILE",
    ...
    parser.add_argument(
        ...
        default="critical",
        help="按告警最高级别决定退出码：critical（默认）/ high / medium / low / none",
    )
    parser.add_argument(
        "-j", "--jobs",
        type=int,
    ...
    try:
        ...
        print(f"\n❌ {sanitize_report_text(e)}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(f"\n❌ 分析流程失败: {sanitize_report_text(e)}", file=sys.stderr)
        sys.exit(1)
    ...
    if run_result.parse_errors:
        ...
        for item in run_result.parse_errors[:10]:
            print(f"  - {sanitize_report_text(item)}", file=sys.stderr)
