...
def _check_distribution(version: str) -> None:
    ...
    with zipfile.ZipFile(wheels[0]) as archive:
        ...
        _assert("bla/output/evidence_report.py" in names, "wheel is missing bla/output/evidence_report.py")
...
'''
        ...
        for name in ("index.html", "report.json", "incident_brief.md", "incident_evidence.csv", "events.csv", "iocs.txt", "report.sarif", "manifest.json"):
        ...
        _assert(bundle_report.get("incident_brief"), "installed CLI report JSON missing incident_brief")
        brief_text = (bundle_dir / "incident_brief.md").read_text(encoding="utf-8")
        _assert("BLA 应急研判摘要" in brief_text, "installed CLI incident_brief.md missing brief title")
        evidence_text = (bundle_dir / "incident_evidence.csv").read_text(encoding="utf-8")
        _assert("used_by" in evidence_text.splitlines()[0], "installed CLI incident_evidence.csv missing used_by column")
        _assert(
        ...
        _assert(manifest.get("summary", {}).get("incident_brief") is True,
                "installed CLI manifest summary missing incident_brief=true")
        ...
        for name in ("index.html", "report.json", "incident_brief.md", "incident_evidence.csv", "events.csv", "iocs.txt", "report.sarif"):
...
def _run_sample_smokes() -> None:
    with tempfile.TemporaryDirectory(prefix="bla-release-smoke-") as tmp:
        ...
        p0_report = json.loads((p0_dir / "report.json").read_text(encoding="utf-8"))
        _assert(p0_report.get("incident_brief"), "P0 report JSON missing incident_brief")
        p0_brief = (p0_dir / "incident_brief.md").read_text(encoding="utf-8")
        _assert("BLA 应急研判摘要" in p0_brief, "P0 incident_brief.md missing brief title")
        ...
        _assert(manifest.get("summary", {}).get("incident_brief") is True,
                "manifest summary missing incident_brief=true")
...
def main() -> int:
    ...
    commands = [
        ...
        [sys.executable, "scripts/eval_detection.py", "--min-recall", "0.95",
         "--min-precision", "0.95", "--max-fp-rate", "0.07"],
