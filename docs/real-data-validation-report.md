...
每个报告目录均包含 `index.html`、`report.json`、`incident_brief.md`、`incident_evidence.csv`、`events.csv`、`iocs.txt`、`report.sarif`、`manifest.json`。
...
- 本小节记录的是当时桌面环境限制：默认 `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3` 曾出现 `python3 -c 'print("hi")'` 卡住，未能在该轮复跑原始 `python3 -m pytest -q` 与 `python3 scripts/release_check.py`。后续已在正常 Python 3.12 环境补跑完整门禁，见下方“回归与验证”。
