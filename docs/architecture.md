...
| Incident Brief | `bla/incident_brief.py` | 将事件和告警压缩为确认事实、主要发现、研判假设、不能确认项和补证建议 | 不重新检测、不自动定案 |
| Output | `bla/output/` | 生成 HTML、JSON、研判 Markdown、CSV、IOC、SARIF 和 bundle | 不重新执行检测逻辑 |
