...
4. 用 `--out` 证明 BLA 不只是终端输出，还能交付 HTML、JSON、CSV、IOC、SARIF 和 manifest。
5. 用 `validate-rules`、`--allowlist`、`benchmark`、`remote-log --help` 证明它具备规则治理、误报压制、性能基准和远程只读采集入口。

发布前仍建议补一个“真实远程 SSH 目标”的授权演示，因为 `remote-log` 的 help 和单元测试已经覆盖接口，但没有真实服务器截图时，远程采集能力的商业说服力会弱一些。
...
| 报告交付 | HTML、JSON、CSV、IOC、SARIF、manifest | `--out /tmp/bla-demo-p0` | 任意日志样本 | 已本地复现 |
...
真实终端截图发布前再补，当前报告先保留要截取的命令和画面重点，避免把未确认截图或临时截图写入 README。
...
- `--out` 同时生成 HTML、JSON、CSV、IOC、SARIF、manifest。
