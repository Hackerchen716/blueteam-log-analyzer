...
v1.3.1 新增 RDP/登录专项模式：用 `--rdp` 从 Windows Security XML/EVTX 中聚焦 4624/4625 登录事件，只保留带远程来源 IP 的登录证据，适合日志量很大时快速筛出 RDP/网络登录线索。感谢 duckpigdog 贡献这个能力方向。

| 能力 | 说明 |
| --- | --- |
| `--rdp` | 仅保留带远程来源 IP 的 Windows 4624/4625 登录事件，不影响通用 Windows 解析器的完整解析 |
| 登录事件明细 | 终端专项分析会展示账号、来源 IP、登录类型、工作站、认证包和失败状态码 |
| 远程工作台支持 | `bla ssh` 工作台中的 `bla FILE --rdp` 也可直接对远程 Windows 日志做本地专项分析 |
| RDP 样例 | `sample_logs/windows_rdp_sample.xml` 可用于快速验证专项模式 |
...
更多变更见 [v1.3.1 发布说明](docs/releases/v1.3.1.md)，历史版本见 [docs/releases](docs/releases/)。
...
| Windows EVTX | `.evtx`（二进制） | 需安装 `python-evtx`；未安装时会明确停止解析并提示转换方案 |
