...
v1.3.6 是一次 Remote Collector 收口和 CI 运行时预适配更新，重点把远程日志子集采集做成可脚本化入口，并保留取证审计记录。

| 方向 | v1.3.6 重点 |
| --- | --- |
| Remote Collector | 新增 `bla remote-log TARGET PATH --tail N --grep TEXT`，远程只读采集日志子集并在本机分析 |
| 取证审计 | `--audit-json` 写出远程采集审计记录，报告 manifest 增加 `remote_collection` |
| Remote Workspace | 交互式 `bla FILE` 同步支持 `--tail`、`--grep` 和 `--audit-json` |
| CI 预适配 | GitHub Actions 启用 Node.js 24 运行时预适配，并把 `remote-log --help` 纳入 smoke |
| 发布校验 | `release_check.py --build` 覆盖版本面、样例分析、Remote Workspace/Collector help、benchmark 和 wheel/sdist 检查 |

更多变更见 [v1.3.6 发布说明](https://github.com/Hackerchen716/blueteam-log-analyzer/blob/main/docs/releases/v1.3.6.md)，历史版本见 [docs/releases](https://github.com/Hackerchen716/blueteam-log-analyzer/tree/main/docs/releases)。
...
║                    Version 1.3.6  |  100% Offline  |  No AI                  ║
╚══════════════════════════════════════════════════════════════════════════════╝
