...
- **还原过程**：用 ATT&CK 阶段拓扑和 Incident 攻击拓扑串起入口、执行、提权、持久化、横向移动、外联等行为。
...
v1.3.7 是一次终端报告可读性和产品归属更新，重点把攻击链从阶段清单升级成拓扑化案件路径，并在 CLI 横幅标明“南京澄安科技有限公司出品”。

| 方向 | v1.3.7 重点 |
| --- | --- |
| CLI 横幅 | 终端报告开头展示 BLA 名称、南京澄安科技有限公司出品、版本、离线/No AI 属性 |
| 攻击阶段拓扑 | ATT&CK 阶段按应急 kill chain 顺序展示，不再只是平铺清单 |
| Incident 攻击拓扑 | 案件视图按源 IP/资产/账号 -> 阶段 -> 关键事件分支还原路径 |
| 演示案例库 | 新增演示案例库与功能覆盖报告，整理公开样本、复现命令和发布前缺口 |
| 发布校验 | 继续覆盖版本面、规则校验、样例分析、Remote Workspace/Collector help、benchmark 和报告包检查 |

更多变更见 [v1.3.7 发布说明](https://github.com/Hackerchen716/blueteam-log-analyzer/blob/main/docs/releases/v1.3.7.md)，历史版本见 [docs/releases](https://github.com/Hackerchen716/blueteam-log-analyzer/tree/main/docs/releases)。
...
```bash
# 脚本化远程采集：只读取最后 1000 行中包含关键词的内容，并在本机分析
bla remote-log root@server.example.com /var/log/auth.log --tail 1000 --grep "Failed password" --out case-auth --audit-json case-auth-audit.json --exit-on none
...
bla@root@server.example.com:/var/log/nginx$ bla access.log --tail 2000 --grep " 500 " --out case-nginx --exit-on none
```
...
tail FILE [N]
bla FILE [--out DIR]
bla FILE --tail N --grep TEXT [--audit-json FILE]
...
BLA 的发布前演示案例已整理成独立报告，覆盖 Linux auth.log、Web access.log、Windows XML/EVTX、HVV/重保 P0 多源关联、报告包、规则校验、误报压制、远程只读采集、日志源矩阵和性能基准。

完整案例库、复现命令、公开数据源和发布前不足见 [BLA 演示案例库与功能覆盖报告](https://github.com/Hackerchen716/blueteam-log-analyzer/blob/main/docs/demo-cases.md)。
...
║                           南京澄安科技有限公司出品                           ║
║                     Version 1.3.7 | 100% Offline | No AI                     ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
⛓ ATT&CK 攻击链分析
  ● 攻击阶段拓扑  按 ATT&CK / 应急 kill chain 顺序排列
  ├─ 初始访问  4 个事件  T1190
  ├─ 主机失陷  1 个事件  T1505.003
  ├─ 命令控制  2 个事件  T1071.004, T1071
  └─ 数据外传  1 个事件  T1041
...
完整演示案例库和功能覆盖矩阵见 [BLA 演示案例库与功能覆盖报告](https://github.com/Hackerchen716/blueteam-log-analyzer/blob/main/docs/demo-cases.md)。
