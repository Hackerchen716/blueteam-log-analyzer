...
v1.3.4 是一次大日志性能和 P0 解析结构治理更新，重点是减少整文件读入、降低 detector 多轮扫描压力，并把 P0 厂商差异继续收敛到 adapter 路由。

| 方向 | v1.3.4 重点 |
| --- | --- |
| 大文件解析 | Windows XML 文件入口按 `<Event>...</Event>` 块流式解析，避免大型 XML 导出被一次性读入 |
| P0 流式路径 | P0 CSV 逐行解析，P0 JSON object / array / JSONL 文件入口增量解码 |
| P0 结构治理 | WAF、VPN、EDR、DNS、代理、防火墙、堡垒机和应用日志按 adapter 元数据路由 |
| 检测热路径 | Detector Registry 增加候选事件 selector，默认检测器先按 tag、event_id、category 预筛 |
| 解析可靠性 | Windows XML 尾部截断事件会计入 `parse_errors`，P0 JSON 坏记录会保留已解析事件并暴露错误计数 |
| 发布校验 | `release_check.py --build` 覆盖版本面、样例分析、Remote Workspace help、benchmark 和 wheel/sdist 检查 |

更多变更见 [v1.3.4 发布说明](https://github.com/Hackerchen716/blueteam-log-analyzer/blob/main/docs/releases/v1.3.4.md)，历史版本见 [docs/releases](https://github.com/Hackerchen716/blueteam-log-analyzer/tree/main/docs/releases)。
...
║                    Version 1.3.4  |  100% Offline  |  No AI                  ║
╚══════════════════════════════════════════════════════════════════════════════╝
